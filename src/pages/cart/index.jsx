...
const Cart = () => {
  ...
  const addOrderUser = async () => {
    try {
      const res = await orderUser(user.token, order.ongkir, order.idAddress, order.subtotal, order.total);
          ...
          <div className="cart_wrapper">
            <>
              <Stepper active={active} onStepClick={setActive} breakpoint="sm">
                <Stepper.Step label="First step" description="Konfirmasi Orderan" allowStepSelect={active > 0}>
                  <Checkout cart={cart} setSubtotal={setSubtotal} />
                </Stepper.Step>
                <Stepper.Step label="Second step" description="Konfirmasi Alamat" allowStepSelect={active > 1}>
                  <AddressConfirm isSetActive={isSetActive} setIdAddress={setIdAddress} />
                </Stepper.Step>
                <Stepper.Step label="Final step" description="Konfirmasi Pembayaran" allowStepSelect={active > 2}>
              ...
              <Group position="center" mt="xl" mb="xl">
                ...
                <Button style={{ backgroundColor: "#1f9ca4" }} onClick={nextStep} disabled={isActive}>
                  Selanjutnya
                </Button>
              </Group>
            </>
